...
class BinarySearchTree extends VisualisationItem {
	constructor(itemData) {
		...
		super(
			...
			itemData,
			{
                ...
                "num_elements" : 0,
                "array"        : Array.from({length}, (v, i) => new BinarySearchTree.WrapElement(length, i)), // Array of Array nodes
            } 
		);
	}
	...
	async* insert(value) {
		...
		if (this.state.num_elements >= MAX_SIZE)
			return { success:false, message:`The Binary Search Tree is full!` };
		...
		while (cvalue !== null) {
			...
			if (nextIndex < 0 || nextIndex >= MAX_SIZE)
				return { success:false, message:"Binary Search Tree cannot grow beyond the current max-depth!" };
		...
		/***** Insert the Value  *****/
		console.log({currentElement, cindex, cvalue})
		...
		return { success:true, message:`Inserted ${value} to the Binary Search Tree!` };
	}

	async* remove(value) {
		if (this.state.num_elements <= 0)
			return { success:false, message:`The Binary Search Tree is empty!` };
		...
		if (!found)
			return { success:false, message:`Could not find ${value} in the Binary Search tree!` };
		...
		return { success:true, message:`Found and removed ${value} fro the Binary Search Tree!` };
	}
	...
	async* search (value) {
		...
		return { success:found, message:`${fstr} ${value} in the Binary Search tree!` };
	}
